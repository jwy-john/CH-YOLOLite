...
class GSConvns(GSConv):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):
        super().__init__(c1, c2, k=k, s=s, p=p, g=g, act=True)
        c_ = c2 // 2
...
#  RFB_U          T40    就是你了,效果嘎嘎好
class RFB_U(nn.Module):  
    def __init__(self, c1, c2):
        super(RFB_U, self).__init__()
        c_ = int(c2 * 0.5)
        self.con1x1_1 = Conv(c1, c_, k=1)      #   256  --->  128
        self.con1x1_128 = Conv(c_, c_ , k=1)      #   128  --->  64
        self.con1x1_64_128 = Conv(int(c_ * 0.5), c_ , k=1)      #   128  --->  64
        self.con3x3_64 = Conv(c_, int(c_ * 0.5) , k=3)      #   128  --->  64
        self.con5x5_64 = Conv(c_, int(c_ * 0.5) , k=5)      #   128  --->  128
        self.con3_1 = nn.Conv2d(c_, c_, kernel_size=3, dilation=1, padding=1)        #   128 
        self.con3_2 = nn.Conv2d(int(c_ * 0.5), int(c_ * 0.5), kernel_size=3, dilation=3, padding=3)
        self.con3_3 = nn.Conv2d(int(c_ * 0.5), int(c_ * 0.5), kernel_size=3, dilation=5, padding=5)
        
        self.cbam = CBAMBlock(channel = c2)
        
        self.bn = nn.BatchNorm2d(c2)   # 64

        self.con1x1_256 = Conv(c1, c2, k=1)
        
    def forward(self, x):
        
        x1 = self.con1x1_1(x)    # 128
        x11 = self.con1x1_128(x1)    #  128
        x111 = self.con3_1(x1)    #  128
        
        x2 = self.con1x1_1(x)    # 128
        x2 = x2 + x111
        x22 = self.con3x3_64(x2)    #  64
        x222 = self.con3_2(x22)    #  64
        x222_ = self.con1x1_64_128(x222) # 128
        
        x3 = self.con1x1_1(x)    # 128
        x3 = x3 + x222_
        x33 = self.con5x5_64(x3)  #  64
        x333 = self.con3_3(x33)    #  128
    
        c123 = torch.cat([x111 , x222 , x333], dim=1)    # 256   
        c123 = self.cbam(c123)
        out123 = self.con1x1_256(c123)
        out = out123 + x
        out = self.bn(out)
        return out
...
class carafeAndReduceChannels(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(carafeAndReduceChannels, self).__init__()
        self.CARAFE = CARAFE(in_channels,1,5)
...
class Detail_Fusion_cat(nn.Module):
    def __init__(self, c1):
        super(Detail_Fusion_cat, self).__init__()
        c_ = int(c1 * 0.5)
        self.con1x1 = Conv(2 * c1 , c1 , k=1)      #   128  --->  64
        self.con1x1_detail = Conv(c_, c1 , k=1)
        self.cbam = CBAMBlock(channel = 2*c1)
        self.mp = nn.MaxPool2d(kernel_size=2, stride=2)
        self.bn = nn.BatchNorm2d(c1)   # 128
        
        
    def forward(self, x):
        
        x01 = torch.cat((x[0], x[1]), dim=1)
        x01 = self.con1x1(x01)

        x2 = self.con1x1_detail(self.mp(x[2]))
        out = torch.cat((x01, x2), dim=1)
        out = self.cbam(out)
        return out
...
class ASFF_3(nn.Module):
    ...
    def forward(self, x):
        level_1_weight_v = self.weight_level_1(x[0])
        level_2_weight_v = self.weight_level_2(x[1])
        level_3_weight_v = self.weight_level_3(x[2])

        levels_weight_v = torch.cat((level_1_weight_v, level_2_weight_v, level_3_weight_v), 1)
        levels_weight = self.weight_levels(levels_weight_v)
        levels_weight = F.softmax(levels_weight, dim=1)

        fused_out_reduced = x[0] * levels_weight[:, 0:1, :, :] + \
                            x[1] * levels_weight[:, 1:2, :, :] + \
                            x[2] * levels_weight[:, 2:, :, :]
