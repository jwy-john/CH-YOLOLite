...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 ...
                 Yolov7_Tiny_E_ELAN_DBB, DiverseBranchBlock, VoVGSCSP, GSConvns, RFB_U, carafeAndReduceChannels]:
            c1, c2 = ch[f], args[0]
            ...
            # c1 = ch[f[0]]
            c2 = max([ch[x] for x in f])   # f是第几层，获取的是它在网络中的列表下标
        elif m is Detail_Fusion_cat:
            c1 = max([ch[x] for x in f])
            c2 = 2 * max([ch[x] for x in f])
            # print(c1)
            args = [c1]
